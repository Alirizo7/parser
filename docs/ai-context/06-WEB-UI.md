...
4. При EXTRACTED/DONE detail показывает ревью-таблицу; при DONE — карточки
   скачивания четырёх docx (эмеральд) + **пяти xlsx-протоколов (синие)**. Имена docx:
   `5_1б.docx`, `6_5_заключение.docx`, `6_4_йиғма_қайднома.docx`,
   `6_6_тадбирлар_режаси.docx`; xlsx:
...
Колонка «Шикастланиш» ходит в `injury_risk_class_6_4` (п.2.3 карты) — ЕДИНОЕ поле
для 6_5 (c18) и 6_4: оператор правит один раз, меняются оба документа. Отображаемое
значение считает `VALUE_RESOLVERS[path]` (для этой колонки — `injury_risk_value`),
чтобы у батчей, извлечённых до появления п.2.3, в ячейке было ровно то, что уйдёт в
документ (фолбэк-эвристика), а не пусто; правка всё равно пишется в
`injury_risk_class_6_4`. Флаг `injury_risk_heuristic` подсвечивает колонку ТОЛЬКО
когда сработал фолбэк (раньше стоял у всех записей — колонка была вечно жёлтой).
...
⚠️ Мелочи UI: на дашборде быстрые кнопки скачивания 5_1б, 6_5 и 6_6 (6_4 — только
на detail); степпер этапов в `_progress.html` подсвечивается подстрочным матчем
