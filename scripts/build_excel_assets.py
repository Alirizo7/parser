...
экспертиза Қарши») делаем ``attestation/assets/template_excel_N.xlsx``:
    * оставляем шапку (строки 1..~25) + заголовки колонок + ОДИН прото-заголовок
      группы + ОДИН прото-блок рабочего места;
...
# group_row — прото-заголовок подразделения;
...
CFG = {
    ...
    5: dict(src="5_elektromagnitnie",    managed_start=24, group_row=27, block_start=28, proto_len=16),
}
